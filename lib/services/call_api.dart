...
import 'package:money_tracking/Utils/env.dart';
...
class CallApi {
  static Future<List<User>> CheckLoginApi(User user) async {
    final responseData = await http.post(
      Uri.parse("${Env.baseUrl}/money_tracking_api/apis/check_login_api.php"),
  ...
  static Future<List<User>> RegisterAPI(User user) async {
    final responseData = await http.post(
      Uri.parse(
          "${Env.baseUrl}/money_tracking_api/apis/insert_new_user_api.php"),
      body: jsonEncode(user.toJson()),
      headers: {"Content-Type": "application/json"},
    );
    if (responseData.statusCode == 200) {
  ...
  static Future<List<Money>> getMoneyAPI(String? userId) async {
    final responseData = await http.post(
      Uri.parse("${Env.baseUrl}/money_tracking_api/apis/get_money_api.php"),
      body: jsonEncode(
        {"userId": userId},
      ),
      headers: {"Content-Type": "application/json"},
  ...
  static Future<List<Money>> AddMoneyApi(Money money) async {
    final responseData = await http.post(
      Uri.parse("${Env.baseUrl}/money_tracking_api/apis/add_money_api.php"),
